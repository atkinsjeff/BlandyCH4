...
methane <- read.csv("methane_raw_data.csv", header=TRUE, fill=TRUE)
...
methane_sort <- methane[order(methane$date, methane$transect, methane$distance, methane$sample),]

meth <- subset(methane_sort, select = -c(comment))
meth <- subset(methane_sort, select = -c(run))
...
write.csv(df.meth.lm, file="meth_lm_output.csv", row.names=TRUE)
